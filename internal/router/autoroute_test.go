...
// warmBody builds a request whose raw size clears flapMinBytes, i.e. one
// with a prefix cache worth protecting.
func warmBody(text string) []byte {
	// Padding sits in an earlier assistant turn so the trailing message is
	// still a fresh user turn — the only kind that re-classifies.
	return body(`{"role":"user","content":"earlier work"},
	  {"role":"assistant","content":"` + repeat("x", flapMinBytes) + `"},
	  {"role":"user","content":"` + text + `"}`)
}

// A classifier that wants a cheaper tier mid-session is asking to abandon
// a warm prefix cache on the current provider and re-prime it on another.
// On a large conversation that costs far more than the cheaper tier saves.
func TestAutoRouteSuppressesDownshiftOnLargeContext(t *testing.T) {
	calls := 0
	a := newAuto("deep", nil, &calls)
	a.route(context.Background(), testRule(), nil, warmBody("plan the migration"), "s1", nil)

	a.classify = func(ctx context.Context, rule config.RouteRule, cfg *config.Config, summary string) (string, error) {
		calls++
		return "light", nil
	}
	alias, tier, reason := a.route(context.Background(), testRule(), nil, warmBody("now rename the symbol"), "s1", nil)
	if tier != "deep" || alias != "opus" {
		t.Errorf("downshift not suppressed: alias=%s tier=%s", alias, tier)
	}
	if reason != "flap:light→deep" {
		t.Errorf("reason = %q, want the flap note", reason)
	}
}

// Escalation is always allowed: a harder turn is worth a cold prefix.
func TestAutoRouteAllowsUpshift(t *testing.T) {
	calls := 0
	a := newAuto("light", nil, &calls)
	a.route(context.Background(), testRule(), nil, warmBody("rename this"), "s1", nil)

	a.classify = func(ctx context.Context, rule config.RouteRule, cfg *config.Config, summary string) (string, error) {
		calls++
		return "deep", nil
	}
	_, tier, reason := a.route(context.Background(), testRule(), nil, warmBody("now redesign the scheduler"), "s1", nil)
	if tier != "deep" {
		t.Errorf("upshift suppressed: tier=%s", tier)
	}
	if reason != "" {
		t.Errorf("reason = %q, want none for an escalation", reason)
	}
}

// A short conversation has nothing expensive to re-prime, so the
// classifier gets its way.
func TestAutoRouteAllowsDownshiftOnSmallContext(t *testing.T) {
	calls := 0
	a := newAuto("deep", nil, &calls)
	a.route(context.Background(), testRule(), nil, body(`{"role":"user","content":"plan it"}`), "s1", nil)

	a.classify = func(ctx context.Context, rule config.RouteRule, cfg *config.Config, summary string) (string, error) {
		calls++
		return "light", nil
	}
	_, tier, _ := a.route(context.Background(), testRule(), nil, body(`{"role":"user","content":"now rename x"}`), "s1", nil)
	if tier != "light" {
		t.Errorf("small-context downshift suppressed: tier=%s", tier)
	}
}

// Once the upstream cache has expired there is nothing left to protect.
func TestAutoRouteAllowsDownshiftAfterCooldown(t *testing.T) {
	calls := 0
	a := newAuto("deep", nil, &calls)
	a.route(context.Background(), testRule(), nil, warmBody("plan the migration"), "s1", nil)
	...
	a.mu.Lock()
	prev := a.cache["s1"]
	prev.at = prev.at.Add(-2 * flapCooldown)
	a.cache["s1"] = prev
	a.mu.Unlock()
	...
	_, tier, _ := a.route(context.Background(), testRule(), nil, warmBody("now rename the symbol"), "s1", nil)
	if tier != "light" {
		t.Errorf("downshift still suppressed past the cache TTL: tier=%s", tier)
	}
}

// A rule with its own tier names has no known ordering, so flap control
// stays out of the way rather than guessing which way is "down".
func TestShouldHoldTierIgnoresUnrankedTiers(t *testing.T) {
	prev := decision{tier: "custom-a", at: time.Now()}
	if shouldHoldTier(prev, "light", flapMinBytes*2) {
		t.Error("held against an unranked previous tier")
	}
	if shouldHoldTier(decision{tier: "deep", at: time.Now()}, "custom-b", flapMinBytes*2) {
		t.Error("held against an unranked chosen tier")
	}
}

// The hold is a debounce, not a freeze: a session that has genuinely
// moved to lighter work asks again on the next turn and gets it. One held
// turn is enough to absorb an alternating classifier, and the tier is not
// pinned for the rest of the session.
func TestAutoRouteDownshiftsOnSecondVote(t *testing.T) {
	calls := 0
	a := newAuto("deep", nil, &calls)
	a.route(context.Background(), testRule(), nil, warmBody("plan the migration"), "s1", nil)
	...
	if _, tier, _ := a.route(context.Background(), testRule(), nil, warmBody("rename a"), "s1", nil); tier != "deep" {
		t.Fatalf("first cheap turn should be held, got %s", tier)
	}
	_, tier, reason := a.route(context.Background(), testRule(), nil, warmBody("rename b"), "s1", nil)
	if tier != "light" {
		t.Errorf("second consecutive cheap turn should move: tier=%s reason=%q", tier, reason)
	}
}

// An alternating classifier never accumulates two consecutive cheap votes,
// so it never gets to pay for a re-prime.
func TestAutoRouteAlternationNeverDownshifts(t *testing.T) {
	calls := 0
	want := "deep"
	a := &autoRouter{
		cache: map[string]decision{},
	...
	a.route(context.Background(), testRule(), nil, warmBody("plan it"), "s1", nil)
	for i, w := range []string{"light", "deep", "light", "deep", "light"} {
		want = w
		_, tier, _ := a.route(context.Background(), testRule(), nil, warmBody(fmt.Sprintf("turn %d", i)), "s1", nil)
		if tier != "deep" {
			t.Fatalf("turn %d (%s): tier=%s, want deep throughout an alternation", i, w, tier)
		}
	}
}
