...
Three more things keep a session from spending window and money on nothing:
...
- **Prefix-cache affinity.** OpenAI-dialect upstreams cache by prefix, but need a hint to route a session's turns back to the machine holding it. `agentic providers add … --prompt-cache-key` sends one per session. Opt-in: some strict OpenAI-compatible servers reject unknown body fields. `agentic cost` reports the cache hit rate so the effect is measurable rather than assumed.
- **Tier-flap control.** Switching models mid-session abandons a warm prefix cache and re-primes the whole conversation at full input price — which at a large context dwarfs what a cheaper tier saves on one turn. A downshift now has to be worth that: it happens outright when the prefix is short or the cache has gone cold, and otherwise has to be asked for on two consecutive turns. That is a debounce, not a freeze — an alternating classifier never accumulates two cheap votes, while a session that really has moved to lighter work moves after one held turn. Escalation is never delayed.
